...
function killer.spawnRandom()
    ...
    while not validPosition and attempts < maxAttempts do
        ...
        for _, s in ipairs(gameReference.stars) do
            ...
            -- Calculate closest point on the killer's rectangle to the star
            local closestX = math.max(leftEdge, math.min(starX, rightEdge))
            local closestY = math.max(topEdge, math.min(starY, bottomEdge))

            -- Compute distance
            local distance = math.sqrt((closestX - starX)^2 + (closestY - starY)^2)
    ...
    if not validPosition then
        print("Warning: Unable to place a killer with valid spacing after multiple attempts")
        return -- Abort spawning if we can't find a good spot
    end
    ...
    fixture:setRestitution(killerRestitution)
